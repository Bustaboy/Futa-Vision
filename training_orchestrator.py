...
def stage_partner_lora_artifact(
    name: str,
    trigger_word: str,
    output_dir: str | Path = "library/partners",
    general_physics_base_lora: str | Path = DEFAULT_OUTPUT_DIR / "general_physics_v1.0.safetensors",
    score_average: float = 0.0,
    save_as_fixed_male: bool = False,
) -> dict[str, Any]:
    """Stage Phase 1 partner/fixed-male LoRA metadata after scoring approval.

    Real partner training will be wired to Ostris once the Phase 2 image/video
    generation pipeline can produce curated datasets.  For Phase 1, this helper
    still enforces the important integration contract: character LoRAs are
    created *on top of* the Phase 0.5 General Physics Base LoRA, use low-VRAM
    hardware defaults, and produce versioned paths/metadata for the library.
    """

    settings = hardware_check.get_low_vram_settings()
    settings["use_low_vram"] = True
    settings["batch_size"] = 1
    settings["mixed_precision"] = "fp8"
    settings["quantization"] = "fp8/int8"

    character_type = "fixed_male" if save_as_fixed_male else "partner"
    safe_name = re.sub(r"[^a-z0-9]+", "_", name.lower()).strip("_") or character_type
    root = Path(output_dir) / ("fixed_male" if save_as_fixed_male else safe_name)
    root.mkdir(parents=True, exist_ok=True)

    counter = 1
    version = "v1.0"
    while (root / f"{safe_name}_{version}.safetensors").exists():
        counter += 1
        version = f"v1.{counter - 1}"

    lora_path = root / f"{safe_name}_{version}.safetensors"
    metadata_path = root / f"{safe_name}_{version}_metadata.json"
    config_path = root / f"{safe_name}_{version}_ostris_config.yaml"
    created_at = datetime.now(UTC).replace(microsecond=0).isoformat()

    config = f"""# Auto-generated by Futa-Vision Phase 1.
# TODO Phase 1 production: replace placeholder artifact staging with a full Ostris partner training job.
job: extension
config:
  name: {safe_name}_{version}
  process:
    - type: sd_trainer
      training_folder: {root.as_posix()}
      network:
        type: lora
        linear: {settings.get('rank_default', 8)}
        linear_alpha: {settings.get('rank_default', 8)}
      base_lora: {Path(general_physics_base_lora).as_posix()}
      train:
        batch_size: 1
        dtype: {settings.get('mixed_precision', 'fp8')}
        quantization: {settings.get('quantization', 'fp8/int8')}
meta:
  ...
  general_physics_base_lora: {Path(general_physics_base_lora).as_posix()}
"""
    config_path.write_text(config, encoding="utf-8")
    if not lora_path.exists():
        lora_path.write_bytes(b"FUTA_VISION_PHASE_1_PLACEHOLDER_CHARACTER_LORA\n")
    ...
    metadata = {
        ...
        "general_physics_base_lora": str(Path(general_physics_base_lora).resolve()),
        "low_vram_settings": settings,
        "notes": [
            "Character LoRA is staged on top of the Phase 0.5 General Physics Base LoRA.",
            "TODO Phase 1: replace placeholder bytes with real Ostris-trained safetensors after approved dataset creation.",
            "Default local philosophy remains 720p generation followed by final upscale.",
        ],
    }
